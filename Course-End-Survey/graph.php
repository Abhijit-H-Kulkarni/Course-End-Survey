<?php
	...
	$c4=0;
	$c5=0;
	$query="SELECT * FROM `umadevi` WHERE `value`=5;";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){$c1++;}
	$query="SELECT * FROM `umadevi` WHERE `value`=4;";
	$res=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($res)){$c2++;}
	$query="SELECT * FROM `umadevi` WHERE `value`=3;";
	$res1=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($res1)){$c3++;}
	$query="SELECT * FROM `umadevi` WHERE `value`=2;";
	$res2=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($res2)){$c4++;}
	$query="SELECT * FROM `umadevi` WHERE `value`=1;";
...
<script type="text/javascript">';
 echo "google.charts.load('current', {'packages':['corechart']});
	...
	echo '<center><form action="teachersfpage.php"><input type="submit" value="HOME" /></form></center>';
	echo '</body>';
	?>